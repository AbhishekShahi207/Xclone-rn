...
export const createPost = asyncHandler(async(req,res)=>{
  ...
  if(!user) return res.status(404).json({error:"User not found"})
    ...
    //this is the response image url from cloudinary upload
    let imageUrl=""
...
export const likePost = asyncHandler(async(req,res)=>{
  ...
  if(!post || !user) return res.status(404).json({error:"Post not found"})
...
export const deletePost =asyncHandler(async(req,res)=>{
  ...
  const post = await Post.findById({postId})
  const user=await Post.findOne({clerkId:userId})

  //only user can delete its post
  if(post.user.toString() !== user._id.toString()){
    return res.status(403).json({error:"You can only delete your own post"})
  }

//delete all the comment of that post to make storage efficient
